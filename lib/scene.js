...
const activePosition = { x: width, y: height }
...
export const scene = (opts = {}) => (Wrap) => {
  ...
  class Scene extends Component {
    static defaultProps = {
      onGestureClosed: () => { console.log('onGestureClosed') }
    }
    ...
    constructor(props, context) {
      ...
      const replace = isSet(userOpts.replace, opts.replace)
    ...
    _handleStartShouldSetPanResponder = (evt, gestureState) => {
      ...
      switch (side) {
        case Side.B:
          ...
          break
        case Side.T:
          if (height - threshold > pageY) {
            return false
          }
          break
        case Side.L:
          if (width - threshold > pageX) {
            return false
          }
          break
        case Side.R:
    ...
    setSceneStatus = (sceneStatus) => {
      ...
      if (sceneStatus === SceneStatus.Activating) {
        Animated.spring(this.state.opacity, { toValue: 1 }).start()
      }
      else if (sceneStatus === SceneStatus.Deactivating) {
    ...
    componentDidMount() {
      this.open(() => {
        this.props.onOpen(this.state.reset, this.state.replace)
        this.props.onDragCancel()
      })
    }
